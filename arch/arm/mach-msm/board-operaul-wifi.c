...
#include "board-operaul-wifi.h"

int operaul_wifi_power(int on);
int operaul_wifi_reset(int on);
int operaul_wifi_set_carddetect(int on);
...
static void *operaul_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init operaul_init_wifi_mem(void)
{
	int i;
...
static struct resource operaul_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data operaul_wifi_control = {
	.set_power      = operaul_wifi_power,
	.set_reset      = operaul_wifi_reset,
	.set_carddetect = operaul_wifi_set_carddetect,
	.mem_prealloc   = operaul_wifi_mem_prealloc,
	.get_mac_addr	= operaul_wifi_get_mac_addr,
	.bus_scale_table    = &wlan_bus_scale_pdata,
};

static struct platform_device operaul_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(operaul_wifi_resources),
	.resource       = operaul_wifi_resources,
	.dev            = {
		.platform_data = &operaul_wifi_control,
	},
};

static unsigned operaul_wifi_update_nvs(char *str)
{
...
int operaul_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"operaul_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
int __init operaul_wifi_init(void)
{
	...
	ret = platform_device_register(&operaul_wifi_device);
	return ret;
}
